<template>
  <div class="location-page bg-poseidon-50 min-h-screen p-6">
    <div class="container mx-auto">
      ...
      <!-- 地點標題 -->
      <h1 class="text-3xl font-bold text-teal-800 mb-6">{{ locationName }} 潛水服務</h1>

      <!-- 載入中提示 -->
      <div v-if="loading" class="bg-white rounded-lg shadow-md p-8 text-center">
        <p class="text-lg text-gray-600">正在載入 {{ locationName }} 的潛水服務...</p>
      </div>
      ...
      <div v-else-if="!services.length" class="bg-white rounded-lg shadow-md p-8 text-center">
        <p class="text-lg text-gray-600">目前 {{ locationName }} 地點尚無潛水服務資料。</p>
      </div>
...
async function fetchLocationServices() {
  ...
  try {
    ...
    // 從 Supabase 獲取特定地點的服務數據
    const { data, error: supabaseError } = await $supabase
      .from("services")
      .select("*")
      .eq("location", locationId.value);
    ...
    services.value = data || [];
    console.log(`已載入 ${locationName.value} 地點的服務數據:`, services.value);
  } catch (err) {
