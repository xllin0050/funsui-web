<template>
  <div class="relative h-full w-full flex-1 px-4 py-8">
    ...
    <div v-if="bookings.length === 0" class="mt-8 text-center text-gray-400">
      目前沒有預約紀錄
    </div>

    <Modal :visible="showModal" @close="closeModal">
      <template #header> 預約詳情 </template>
      <template #body>
        <div v-if="selectedBooking" class="space-y-4">
          <div class="flex justify-between">
            <span class="font-semibold">地點：</span>
            <span
              >{{ selectedBooking.site }} ({{ selectedBooking.region }})</span
            >
          </div>
          <div class="flex justify-between">
          ...
          <div class="flex justify-between">
            <span class="font-semibold">人數：</span>
            <span>{{ selectedBooking.people }}</span>
          </div>
          <div class="flex justify-between">
      ...
      <template #footer>
        <button
        class="bg-poseidon-500 hover:bg-poseidon-600 rounded px-4 py-2 font-bold text-white"
        @click="closeModal"
        >
          關閉
        </button>
